...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        *)
            ...
            echo ""
            echo "Options:"
            echo "  --all        Deploy all files"
            echo "  --templates  Deploy HTML template files"
            echo "  --js         Deploy JavaScript files"
            echo "  --css        Deploy CSS files"
            echo "  --backend    Deploy Python backend files (requires restart)"
            echo "  --restart    Force container restart"
            echo ""
            echo "Examples:"
            echo "  $0 --all                    # Deploy everything"
            echo "  $0 --templates              # Deploy only HTML files"
            echo "  $0 --js --css               # Deploy JS and CSS files"
            echo "  $0 --backend --restart      # Deploy backend and restart"
            exit 1
            ;;
    esac
done
...
if [ -f "/tmp/.deployment_flag" ]; then
    DEPLOYMENT_DONE=true
    rm -f "/tmp/.deployment_flag"
fi

# Track if any deployment was done
DEPLOYMENT_DONE=false
...
# Only show completion message if deployment was done
